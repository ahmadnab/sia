      ...
      <div className="p-6 lg:p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <button
            ...
            className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm cursor-pointer hover:shadow-md transition-shadow text-left w-full focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2"
            type="button"
          ...
          >
            ...
            <p className="text-sm text-slate-500 mt-1 pointer-events-none">
              {engagementMetrics.activeSurveyCount === 0 
                ? 'No active surveys' 
            ...
            <div className="flex items-center gap-4 pointer-events-none">
              <div className="w-16 h-16 pointer-events-none">
                {riskDistributionData.length > 0 ? (
                  ...
                  <div className="w-full h-full bg-slate-100 rounded-full flex items-center justify-center pointer-events-none">
              ...
              <div className="flex-1 pointer-events-none">
                <div className="flex items-baseline gap-2 pointer-events-none">
                  <p className="text-2xl font-bold text-red-600 pointer-events-none">{riskMetrics.highRiskPercent}%</p>
                  <span className="text-sm text-slate-500 pointer-events-none">high risk</span>
                </div>
                <div className="flex gap-3 mt-1 text-xs text-slate-500 pointer-events-none">
                  ...
                  {riskMetrics.unknownRiskCount > 0 && (
                    <span className="flex items-center gap-1 pointer-events-none" title="No GPA data">
                      <HelpCircle size={12} className="text-slate-400 pointer-events-none" />
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" onClick={() => setShowResponseRateModal(false)}>
            <div className="bg-white rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
              <div className="p-6 border-b border-slate-200 flex items-center justify-between">
                <h2 className="text-xl font-bold text-slate-900">Response Rate Details</h2>
                <button onClick={() => setShowResponseRateModal(false)} className="p-2 hover:bg-slate-100 rounded-lg transition-colors">
                  <X size={20} className="text-slate-500" />
                </button>
              </div>
              <div className="p-6">
                {responseRateDetails.length === 0 ? (
                  <p className="text-slate-500 text-center py-8">No active surveys to display.</p>
                ) : (
                  <div className="space-y-4">
                    {responseRateDetails.map((detail) => (
                      <div key={detail.surveyId} className="border border-slate-200 rounded-lg p-4">
                        <div className="flex items-center justify-between mb-2">
                          <h3 className="font-semibold text-slate-900">{detail.surveyName}</h3>
                          <span className="text-lg font-bold text-teal-600">{detail.rate}%</span>
                        </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" onClick={() => setShowRecentActivityModal(false)}>
            <div className="bg-white rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
              <div className="p-6 border-b border-slate-200 flex items-center justify-between">
                ...
                <button onClick={() => setShowRecentActivityModal(false)} className="p-2 hover:bg-slate-100 rounded-lg transition-colors">
                  <X size={20} className="text-slate-500" />
                </button>
              </div>
              <div className="p-6">
                {recentActivityDetails.length === 0 ? (
                  <p className="text-slate-500 text-center py-8">No activity in the last 7 days.</p>
                ) : (
                  <div className="space-y-4">
                    {recentActivityDetails.map((day, idx) => (
                      <div key={idx} className="border border-slate-200 rounded-lg p-4">
                        <div className="flex items-center justify-between mb-3">
                          ...
                          <span className="text-lg font-bold text-purple-600">{day.count} response{day.count !== 1 ? 's' : ''}</span>
                        </div>
                        <div className="space-y-2">
                          {day.responses.slice(0, 5).map((response, rIdx) => (
                            <div key={rIdx} className="text-sm text-slate-600 pl-4 border-l-2 border-purple-200">
                          ...
                          {day.responses.length > 5 && (
                            <p className="text-sm text-slate-500 pl-4">...and {day.responses.length - 5} more</p>
                          )}
                        </div>
                      </div>
                    ))}
        ...
        {showRiskDistributionModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" onClick={() => setShowRiskDistributionModal(false)}>
            <div className="bg-white rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
              <div className="p-6 border-b border-slate-200 flex items-center justify-between">
                <h2 className="text-xl font-bold text-slate-900">Risk Distribution Details</h2>
                <button onClick={() => setShowRiskDistributionModal(false)} className="p-2 hover:bg-slate-100 rounded-lg transition-colors">
                  <X size={20} className="text-slate-500" />
                </button>
              </div>
              <div className="p-6">
                <div className="grid grid-cols-2 gap-4 mb-6">
                  <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                    ...
                    <p className="text-2xl font-bold text-red-600">{riskMetrics.highRiskCount}</p>
                  ...
                  <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
                    ...
                    <p className="text-2xl font-bold text-amber-600">{riskMetrics.mediumRiskCount}</p>
                  ...
                  <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <h3 className="font-semibold text-green-900">Low Risk</h3>
                    </div>
                    <p className="text-2xl font-bold text-green-600">{riskMetrics.lowRiskCount}</p>
                  ...
                  {riskMetrics.unknownRiskCount > 0 && (
                    <div className="bg-slate-50 border border-slate-200 rounded-lg p-4">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <h3 className="font-semibold text-slate-900">Unknown</h3>
                      </div>
                      <p className="text-2xl font-bold text-slate-600">{riskMetrics.unknownRiskCount}</p>
                ...
                <div className="border-t border-slate-200 pt-4">
                  <h3 className="font-semibold text-slate-900 mb-3">High Risk Students</h3>
                  {riskMetrics.highRiskCount === 0 ? (
                    <p className="text-slate-500 text-sm">No high-risk students at this time.</p>
                  ) : (
